    ...
    return (
      ...
      >
        <AnimatePresence>
          {isHovered && (
            <motion.div
              // Rectangular hover, tight to the label — matches FF's agent trace
              // header (not the oversized pill the shape context defaults to).
              className="absolute inset-0 rounded-md bg-hover pointer-events-none"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
        ...
        <Collapsible.Trigger
          ref={ref}
          className={cn(
            "relative z-10 flex items-center gap-2 rounded-md px-2 py-1 cursor-pointer outline-none select-none",
        ...
        >
          ...
          <div className={cn("flex gap-2.5 px-2 py-1.5", shape.item)}>
            ...
            <div className="flex-1 flex flex-col gap-1 min-w-0">
              ...
              {description && (
                <span className="text-[13px] text-muted-foreground leading-snug whitespace-pre-wrap">
                  {description}
                </span>
              )}
              {children}
